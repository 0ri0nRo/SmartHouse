...
function PowerButton({ isOn, onClick }) {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', alignItems:'center',
      gap:'0.875rem', padding:'1.5rem 1rem' }}>
      <div style={{ position:'relative', width:100, height:100 }}>
        {isOn && (
          <svg viewBox="0 0 100 100" style={{
            ...
            <circle cx="50" cy="50" r="46" stroke="var(--card-temp-accent)"
              strokeWidth="1.5" strokeDasharray="6 4" strokeLinecap="round" fill="none" opacity="0.7"/>
          </svg>
        )}
        <button
          ...
          style={{
            ...
            boxShadow: isOn ? `0 0 20px var(--card-temp-bg)` : 'none',
          }}
        >
          <Power size={26} strokeWidth={1.5}/>
        </button>
      </div>
      <div style={{
        fontFamily: 'var(--font-mono)', fontSize:'1.1rem', fontWeight:500,
        letterSpacing: 4, color: isOn ? 'var(--card-temp-accent)' : 'var(--text-muted)',
...
export default function TemperaturePage() {
  ...
  useEffect(() => {
    ...
    else if (diff < -0.3) setTempStatus(`Above target (${diff.toFixed(1)}°C)`)
    else                  setTempStatus('Target temperature reached ✓')
  }, [currentTemp, targetTemp, thermostatEnabled])

  // ── API ───────────────────────────────────────────────────
  const loadBoilerStatus   = () => api.getBoilerStatus().then(d => setIsOn(d.is_on)).catch(() => {})
  const loadTargetTemp     = () => api.getTargetTemp().then(d => { if (d.target_temperature != null) setTargetTemp(d.target_temperature) }).catch(() => {})
  ...
  const fetchCurrentTemp   = () => api.getSensors().then(d => setCurrentTemp(parseFloat(d.temperature.current))).catch(() => {})
  const syncBoiler         = () => api.thermostatSync().then(() => loadBoilerStatus()).catch(() => {})
  ...
  const handleToggleBoiler = async () => {
    ...
    if (thermostatEnabled) { await api.thermostatOff().catch(() => {}); setThermostatEnabled(false) }
    try {
      const newState = !isOn
      await api.manualBoiler(newState)
      setIsOn(newState)
  ...
  const handleAdjustTemp = async (change) => {
    ...
    setTargetTemp(next)
    api.setTargetTemp(next).catch(() => showToast('Error updating target temperature', 'error'))
  }

  const handleToggleThermostat = async () => {
    const newState = !thermostatEnabled
    try {
      await (newState ? api.thermostatOn() : api.thermostatOff())
  ...
  const handleAddSchedule = async ({ onTime, offTime, days }) => {
    const timespecDays = days.split(',').map(d => d.trim()).map(d => DAY_MAP[d]).filter(d => d !== undefined)
    ...
    try {
      await api.createSchedule({ timespec:`0 ${onM} ${onH} * * ${timespecDays.join(',')}`, is_on:true })
      await api.createSchedule({ timespec:`0 ${offM} ${offH} * * ${timespecDays.join(',')}`, is_on:false })
  ...
  const handleDeleteSchedule = async (id) => {
    ...
    try { await api.deleteSchedule(id); showToast('Schedule deleted'); loadSchedules() }
  ...
  const DailyControls = (
    <>
      {[
        { label:'Month', el: <select className="select" style={{ padding:'0.45rem 0.6rem', fontSize:'0.8rem' }} value={month} onChange={e => setMonth(+e.target.value)}>{MONTHS.map((n,i)=><option key={i} value={i+1}>{n}</option>)}</select> },
        { label:'Year',  el: <select className="select" style={{ padding:'0.45rem 0.6rem', fontSize:'0.8rem' }} value={year}  onChange={e => setYear(+e.target.value)}>{years.map(y=><option key={y}>{y}</option>)}</select> },
      ].map(f => (
      ...
      <div className="field" style={{ flex:'none', justifyContent:'flex-end' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:'0.4rem' }}>
          <input type="checkbox" id="cmp" checked={compareEnabled} onChange={e => setCompareEnabled(e.target.checked)}/>
          <label htmlFor="cmp" style={{ fontSize:'0.75rem', color:'var(--text-secondary)', cursor:'pointer' }}>Compare</label>
        </div>
      </div>
      {compareEnabled && <>
        <div className="field" style={{ flex:1, minWidth:110 }}>
          <label className="field-label">Cmp Month</label>
          <select className="select" style={{ padding:'0.45rem 0.6rem', fontSize:'0.8rem' }} value={compareMonth} onChange={e => setCompareMonth(+e.target.value)}>{MONTHS.map((n,i)=><option key={i} value={i+1}>{n}</option>)}</select>
        </div>
        <div className="field" style={{ flex:1, minWidth:90 }}>
          <label className="field-label">Cmp Year</label>
          <select className="select" style={{ padding:'0.45rem 0.6rem', fontSize:'0.8rem' }} value={compareYear} onChange={e => setCompareYear(+e.target.value)}>{years.map(y=><option key={y}>{y}</option>)}</select>
        </div>
      </>}
      <button className="btn btn--primary btn--sm" style={{ alignSelf:'flex-end', whiteSpace:'nowrap' }}
        onClick={() => loadCharts(month, year, compareEnabled, compareMonth, compareYear)} disabled={loadingCharts}>
        <RefreshCw size={13} style={{ animation:loadingCharts?'spin 0.8s linear infinite':'none' }}/> Update
      </button>
    </>
  )
    ...
    <div className="page animate-fade">
      ...
      <div className="temp-layout">
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:'1.5rem' }}>
          ...
          <div className="card section-card--temp" style={{ border:'1px solid var(--card-temp-border)' }}>
            <div className="card-header">
              ...
              <span className={`badge ${thermostatEnabled ? 'badge--success' : isOn ? 'badge--warning' : 'badge--muted'}`} style={{ marginLeft:'auto' }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'0.75rem', padding:'0 1.25rem 1rem' }}>
              {[
                { label:'Current', val:currentTemp!=null?`${currentTemp.toFixed(1)}°C`:'--°C', bg:'var(--card-temp-bg)', color:'var(--card-temp-accent)' },
                { label:'Target',  val:`${targetTemp.toFixed(1)}°C`,                            bg:'var(--accent-light)', color:'var(--accent)' },
              ].map(b => (
                <div key={b.label} style={{ textAlign:'center', background:b.bg, borderRadius:'var(--radius-md)', padding:'0.75rem' }}>
                  ...
                  <div style={{ fontFamily:'var(--font-mono)', fontSize:'1.4rem', fontWeight:500, color:b.color, margin:'0.25rem 0', lineHeight:1 }}>{b.val}</div>
                </div>
              ))}
            </div>
            ...
            <div style={{ display:'flex', gap:'0.5rem', padding:'0 1.25rem 1rem' }}>
              {[[-0.5,'−0.5°C'],[0.5,'+0.5°C']].map(([val,label]) => (
                <button key={val} className="btn btn--ghost" style={{ flex:1 }} onClick={() => handleAdjustTemp(val)}>
            ...
            {/* Status */}
            <div style={{ margin:'0 1.25rem 1rem', padding:'0.65rem 0.9rem', border:'1px solid var(--border)',
              borderRadius:'var(--radius-md)', background:'var(--bg-surface-2)', display:'flex',
              ...
              <div>
                ...
                <div style={{ fontSize:'0.67rem', color:'var(--text-muted)' }}>{tempStatus}</div>
              </div>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:'0.5rem', padding:'0 1.25rem 1.25rem' }}>
              <button className={`btn ${thermostatEnabled?'btn--danger':'btn--primary'} btn--full`} onClick={handleToggleThermostat}>
                <Thermometer size={15}/> {thermostatEnabled ? 'Disable Thermostat' : 'Enable Thermostat'}
              </button>
              <button className="btn btn--ghost btn--full" onClick={() => api.thermostatSync().then(loadBoilerStatus).then(() => showToast('Synced with Shelly')).catch(() => showToast('Sync failed','error'))}>
          ...
          <div className="card">
            <div style={{ padding:'1rem 1.25rem', borderBottom:'1px solid var(--border)', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
              <div>
                <div style={{ fontFamily:'var(--font-mono)', fontSize:'1.75rem', fontWeight:500, color:'var(--accent)', lineHeight:1 }}>{schedules.length}</div>
                <div style={{ fontSize:'0.65rem', letterSpacing:1, textTransform:'uppercase', color:'var(--text-muted)', marginTop:2 }}>Schedules</div>
              </div>
            ...
            <div style={{ padding:'0.75rem 1.25rem', display:'flex', flexDirection:'column', gap:'0.6rem' }}>
              {Object.keys(groupedSchedules).length === 0 ? (
                <div className="empty-state"><Clock size={22}/><div>No schedules configured</div></div>
              ) : Object.entries(groupedSchedules).map(([key, g]) => {
                const daysStr = key.split(',').map(Number).map(n => DAY_NAMES[n]).join(', ')
                return (
                  <div key={key} style={{ background:'var(--bg-surface-2)', border:'1px solid var(--border)', borderRadius:'var(--radius-md)', padding:'0.85rem' }}>
                    <div style={{ display:'flex', alignItems:'center', gap:'0.4rem', fontFamily:'var(--font-mono)', fontSize:'0.85rem', fontWeight:500, marginBottom:2 }}>
                    ...
                    <div style={{ display:'flex', gap:'0.4rem' }}>
                      {g.on.map(s=>(
                        <button key={s.id} className="btn btn--danger btn--sm" style={{ flex:1, fontSize:'0.65rem' }} onClick={() => handleDeleteSchedule(s.id)}>
                          <Trash2 size={11}/> ON
                        </button>
                      ))}
                      {g.off.map(s=>(
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:'1.5rem' }}>
          ...
          <ChartCard title="Today's Temperature" icon={Sun} iconSection="temp" badge="Real-time">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={todayData}>
                ...
                <Tooltip {...TOOLTIP} formatter={v=>[v!=null?`${v}°C`:'N/A','Temp']}/>
                <Area type="monotone" dataKey="temp" stroke="var(--card-temp-accent)" fill="url(#gt)" strokeWidth={2} dot={{ r:3, fill:'var(--card-temp-accent)', strokeWidth:0 }} connectNulls/>
              </AreaChart>
            </ResponsiveContainer>
          </ChartCard>

          <ChartCard title="Monthly Average" icon={TrendingUp} iconSection="air" badge={`${year}`}>
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={monthlyData}>
                ...
                <YAxis tick={{ fontFamily:'var(--font-mono)', fontSize:9, fill:'var(--text-secondary)' }} unit="°"/>
                <Tooltip {...TOOLTIP} formatter={v=>[v!=null?`${v}°C`:'N/A',`Avg ${year}`]}/>
                <Area type="monotone" dataKey="temp" stroke="var(--card-air-accent)" fill="url(#gm)" strokeWidth={2} dot={{ r:3, fill:'var(--card-air-accent)', strokeWidth:0 }} connectNulls/>
              </AreaChart>
            </ResponsiveContainer>
          </ChartCard>

          <ChartCard title="Daily Analysis" icon={CalendarDays} iconSection="temp" controls={DailyControls}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={dailyData.map((d,i)=>({...d, compare:compareData?compareData[i]:undefined}))} barGap={2}>
                <CartesianGrid strokeDasharray="3 3" stroke="var(--border)"/>
                ...
                <Bar dataKey="temp" name={`${MONTHS[month-1].substring(0,3)} ${year}`} fill="var(--accent)" radius={[3,3,0,0]} maxBarSize={18}/>
                {compareData && <Bar dataKey="compare" name={`${MONTHS[compareMonth-1].substring(0,3)} ${compareYear}`} fill="var(--card-air-accent)" radius={[3,3,0,0]} maxBarSize={18}/>}
              </BarChart>
            </ResponsiveContainer>
          </ChartCard>

          <ChartCard title="Custom Date Range" icon={LineChartIcon} iconSection="temp"
            controls={<>
              {[{label:'Start Date',val:startDate,set:setStartDate},{label:'End Date',val:endDate,set:setEndDate}].map(f=>(
                <div key={f.label} className="field" style={{ flex:1, minWidth:160 }}>
                  <label className="field-label">{f.label}</label>
                  <input type="datetime-local" className="input input--mono" style={{ padding:'0.45rem 0.6rem', fontSize:'0.8rem' }} value={f.val} onChange={e=>f.set(e.target.value)}/>
                </div>
              ))}
              <button className="btn btn--primary btn--sm" style={{ alignSelf:'flex-end', whiteSpace:'nowrap' }} onClick={handleFetchRange} disabled={loadingRange}>
                <Search size={13} style={{ animation:loadingRange?'spin 0.8s linear infinite':'none' }}/> Fetch
              </button>
            </>}
          >
            {rangeData.length === 0 ? (
              <div className="empty-state"><LineChartIcon size={28}/><div>Select a date range and fetch data</div></div>
            ) : (
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={rangeData}>
                  ...
                  <CartesianGrid strokeDasharray="3 3" stroke="var(--border)"/>
                  <XAxis dataKey="time" tick={{ fontFamily:'var(--font-mono)', fontSize:9, fill:'var(--text-secondary)' }} interval="preserveStartEnd"/>
                  <YAxis tick={{ fontFamily:'var(--font-mono)', fontSize:9, fill:'var(--text-secondary)' }} unit="°"/>
                  <Tooltip {...TOOLTIP} formatter={v=>[`${v}°C`,'Avg Temp']}/>
      ...
      {showSchedModal && <ScheduleModal onClose={() => setShowSchedModal(false)} onSave={handleAddSchedule}/>}
      <Toast toast={toast}/>
      <style>{`
        ...
        @media (max-width: 900px) {
          .temp-layout { grid-template-columns: 1fr; }
        }
      `}</style>
    </div>
  )
