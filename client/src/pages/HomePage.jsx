...
// ── Shared widget shell ────────────────────────────────────
// All widgets use the SAME shell so they're always the same height.
// The shell has 3 fixed zones: top (icon+arrow), middle (label+sublabel), bottom (value area).
  ...
  return (
    <div
      ...
      style={{
        ...
        height: '100%',           // stretch to grid cell
      }}
      onMouseEnter={e => e.currentTarget.style.borderColor = 'var(--border-strong)'}
      onMouseLeave={e => e.currentTarget.style.borderColor = 'var(--border)'}
    >
      {/* ── Zone 1: icon + chevron — fixed height ── */}
      <div style={{
      ...
      {/* ── Zone 3: value — fills remaining space ── */}
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
        ...
        <button
          ...
          style={{
            ...
            position: 'relative',
            transition: 'background 0.28s ease, transform 0.1s ease, box-shadow 0.28s ease',
            transform: pressed ? 'scale(0.9)' : 'scale(1)',
            boxShadow: isOn
              ? '0 0 12px rgba(52,199,89,0.4)'
              : '0 0 12px rgba(255,59,48,0.3)',
            opacity: (loading || isOn === null) ? 0.5 : 1,
            flexShrink: 0,
          }}
        >
          <div style={{
  ...
  return (
    <Widget section={section} icon={icon} label={label} sublabel={sublabel} onNavigate={onNavigate}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.6rem', width: '100%' }}>
        {[
          { v: val1, u: unit1, t: tag1, c: color1 },
          { v: val2, u: unit2, t: tag2, c: color2 },
        ].map((item, i) => (
          <div key={i} style={{
            ...
            <div style={{ fontSize: '0.58rem', fontWeight: 600, letterSpacing: '0.6px', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: '0.4rem' }}>
              {item.t}
            </div>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: '1.4rem', fontWeight: 500, color: item.c, lineHeight: 1 }}>
              {item.v ?? '—'}
              {item.u && item.v != null && (
                <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', fontWeight: 400 }}>{item.u}</span>
              )}
            </div>
          </div>
        ))}
...
export default function HomePage() {
  ...
  return (
    <div className="page animate-fade">
      ...
      {/* ── Widget grid — equal-height cells ── */}
      <div style={{
        display: 'grid',
