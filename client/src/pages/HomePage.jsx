...
function Widget({ section, icon: Icon, label, sublabel, onNavigate, children }) {
  return (
    <div onClick={onNavigate} style={{
      ...
      onMouseEnter={e=>{ if(onNavigate) e.currentTarget.style.borderColor='var(--border-strong)' }}
...
function DualWidget({ section, icon, label, sublabel, onNavigate,
  val1, unit1, tag1, color1, val2, unit2, tag2, color2 }) {
  return (
    <Widget section={section} icon={icon} label={label} sublabel={sublabel} onNavigate={onNavigate}>
...
function RefreshBar({ secondsLeft, total }) {
...
function NotifBanner({ onRequest, onDismiss }) {
  return (
    <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between',
      ...
      <div style={{ display:'flex', gap:'0.5rem' }}>
        <button className="btn btn--primary btn--sm" onClick={onRequest}>Enable</button>
        <button className="btn btn--ghost btn--sm" onClick={onDismiss}>Not now</button>
      </div>
    </div>
  )
}
  ...
  const loadAll = useCallback(async (silent=false) => {
    ...
    await Promise.allSettled([
      ...
      Promise.all([api.getBoilerStatus(),api.getThermostatFull()])
        .then(([b,t])=>{setBoilerOn(b.is_on);setThermostat(t.thermostat_enabled)})
        .catch(()=>{}),
    ])
  ...
  return (
    <div className="animate-fade">
      ...
      {showNotifBanner && permission === 'default' && (
        <NotifBanner
          onRequest={async () => { await request(); setShowNotifBanner(false) }}
