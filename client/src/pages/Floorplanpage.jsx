...
import {
  ...
  ChevronRight, MoreHorizontal, Zap, Search,
} from 'lucide-react'
import { api } from '../api'
...
function ClimateTrendCard() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        const merged = Array.from(hourSet)
  ...
  const fetchRange = async () => {
    ...
    try {
      ...
      const timeSet = new Set([
        ...tempRange.map((entry) => entry.hour),
        ...humRange.map((entry) => entry.hour),
      ])

      const merged = Array.from(timeSet)
        .sort((a, b) => new Date(a) - new Date(b))
        .map((hour) => {
          const tempEntry = tempRange.find((entry) => entry.hour === hour)
          const humEntry = humRange.find((entry) => entry.hour === hour)
  ...
  return (
    <GCard style={{ overflow: 'hidden', minHeight: 340 }}>
      <div style={{ padding: '0.75rem 1rem', borderBottom: `1px solid ${T.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '0.75rem', flexWrap: 'wrap' }}>
      ...
      <div style={{ padding: '0.8rem 1rem 0.9rem', borderBottom: `1px solid ${T.border}`, display: 'flex', flexWrap: 'wrap', gap: '0.65rem', alignItems: 'end', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.65rem', alignItems: 'end' }}>
          ...
          <button
            ...
            style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', alignSelf: 'end' }}
          >
            <Search size={12} /> Query history
          </button>
          <button
            className="btn btn--ghost btn--sm"
            onClick={() => { setChartMode('today'); setRangeLabel('Today') }}
            disabled={chartMode === 'today'}
            style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', alignSelf: 'end' }}
          >
            Today
          </button>
        </div>
        <div style={{ fontSize: '0.58rem', color: T.textMuted, fontFamily: T.mono }}>
          {chartMode === 'range' ? rangeLabel : 'Showing today'}
        </div>
      </div>

      <div style={{ height: 280, padding: '0.9rem 0.85rem 1rem' }}>
        {loading ? (
...
function BoilerControlCard({ isOn, thermostat, currentTemp, targetTemp, onToggleBoiler, onToggleThermostat, onAdjustTarget, onOpenBlackout, statusLabel }) {
  ...
  return (
    <GCard style={{ overflow: 'hidden' }}>
      ...
      <div style={{ padding: '0.9rem 1rem 1rem', display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: '0.75rem' }}>
      ...
      <div style={{ padding: '0 1rem 1rem', display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
        ...
        <button className="btn btn--sm btn--ghost" onClick={onToggleThermostat}>
          {thermostat ? 'Disable thermostat' : 'Enable thermostat'}
        </button>
        <button className="btn btn--sm btn--ghost" onClick={onOpenBlackout}>
          Season blackout
        </button>
...
function BoilerBlackoutModal({ onClose, onSaved, showToast }) {
  ...
  useEffect(() => {
    const loadCfg = async () => {
      ...
      try {
        ...
        setStartMonth(data.start_month || 4)
        setStartDay(data.start_day || 1)
        setEndMonth(data.end_month || 9)
        setEndDay(data.end_day || 30)
        setReason(data.reason || 'Boiler disabled during warm season')
        setCurrentlyBlocked(!!data.currently_blocked)
      } catch {
  ...
  const save = async () => {
    ...
    try {
      ...
      onSaved?.()
      showToast('Blackout settings saved', 'success')
      onClose()
    } catch (err) {
      ...
      {loading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: '0.4rem', flexWrap: 'wrap' }}>
                  ...
                  <select style={{ ...selectStyle, minWidth: 56 }} value={startDay} onChange={(e) => setStartDay(+e.target.value)}>
                    {Array.from({ length: daysInMonth(startMonth) }, (_, i) => i + 1).map((d) => <option key={d} value={d}>{d}</option>)}
                  </select>
                </div>
              </div>
              <div>
                ...
                <div style={{ display: 'flex', gap: '0.4rem', flexWrap: 'wrap' }}>
                  ...
                  <select style={{ ...selectStyle, minWidth: 56 }} value={endDay} onChange={(e) => setEndDay(+e.target.value)}>
                    {Array.from({ length: daysInMonth(endMonth) }, (_, i) => i + 1).map((d) => <option key={d} value={d}>{d}</option>)}
                  </select>
                </div>
              </div>
            </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '0.75rem', paddingTop: '0.75rem', borderTop: `1px solid ${T.border}` }}>
            <div style={{ fontSize: '0.68rem', color: T.textMuted, fontFamily: T.mono }}>
              {currentlyBlocked ? 'Active now' : 'Not active now'}
            </div>
...
function LiveValueBadge({ liveApi }) {
  ...
  return (
    <div style={{
      ...
      background: hasValue ? 'rgba(52,211,153,0.07)' : T.surface,
      border: `1px solid ${hasValue ? 'rgba(52,211,153,0.20)' : T.border}`,
    }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.25rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
          <span style={{
            width: 5, height: 5, borderRadius: '50%',
            background: hasValue ? '#34d399' : T.textMuted,
      ...
      {error ? (
        ...
        <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.2rem' }}>
          <span style={{ fontFamily: T.mono, fontSize: '1.25rem', fontWeight: 800, color: T.textPrimary, lineHeight: 1 }}>
            {formatLiveValue(value)}
          </span>
          {liveApi.unit && (
            <span style={{ fontFamily: T.mono, fontSize: '0.68rem', color: T.textSecondary, fontWeight: 500 }}>
              {liveApi.unit}
            </span>
          )}
        </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '0.2rem' }}>
        <span style={{ fontSize: '0.55rem', fontFamily: T.mono, color: T.textMuted, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '55%' }}>
          {liveApi.value_path || 'root'}
        </span>
        {timestamp && (
          <span style={{ fontSize: '0.55rem', fontFamily: T.mono, color: T.textMuted }}>{relativeTs(timestamp)}</span>
        )}
      </div>
    </div>
  )
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        backdropFilter: T.blur,
        WebkitBackdropFilter: T.blur,
        border: `1px solid ${isSelected ? meta.color + '60' : T.glassBd}`,
        borderRadius: T.radius,
        padding: '1.1rem',
        cursor: 'pointer',
        position: 'relative',
    ...
    >
      ...
      {hasApi && (
        <div style={{
          ...
          padding: '2px 7px', borderRadius: T.radiusFull, fontSize: '0.50rem', fontWeight: 700,
          color: '#34d399', fontFamily: T.mono,
          background: 'rgba(52,211,153,0.10)', border: '1px solid rgba(52,211,153,0.22)',
        }}>
          <span style={{ width: 4, height: 4, borderRadius: '50%', background: '#34d399', animation: 'pulse 2s infinite', display: 'inline-block' }} />
          LIVE
        </div>
      )}
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.65rem', marginBottom: '0.85rem', paddingRight: hasApi ? '3rem' : 0 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontWeight: 700, fontSize: '0.85rem', color: T.textPrimary, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            {sensor.name}
          </div>
          <div style={{ fontSize: '0.62rem', color: T.textSecondary, marginTop: 2 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem', marginBottom: '0.85rem' }}>
          {sensor.temperature != null && (
            <div style={{ background: 'rgba(249,115,22,0.08)', borderRadius: T.radiusSm, padding: '0.5rem', textAlign: 'center' }}>
              ...
              <div style={{ fontFamily: T.mono, fontSize: '1.1rem', fontWeight: 800, color: sensor.temperature > 28 ? '#f87171' : T.textPrimary }}>
          ...
          {sensor.humidity != null && (
            <div style={{ background: 'rgba(56,189,248,0.08)', borderRadius: T.radiusSm, padding: '0.5rem', textAlign: 'center' }}>
              ...
              <div style={{ fontFamily: T.mono, fontSize: '1.1rem', fontWeight: 800, color: sensor.humidity > 75 ? '#f87171' : T.textPrimary }}>
      ...
      {!hasApi && sensor.last_seen && (
        <div style={{ fontSize: '0.58rem', color: T.textMuted, fontFamily: T.mono, marginBottom: '0.75rem' }}>
      ...
      <div style={{ display: 'flex', gap: '0.4rem' }}>
        <button
          onClick={e => { e.stopPropagation(); onConfigureApi(sensor) }}
          style={{
            flex: 1, fontSize: '0.65rem', fontWeight: 600, fontFamily: T.sans,
            display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.3rem',
            padding: '0.4rem',
            color: hasApi ? '#34d399' : T.textSecondary,
            borderRadius: T.radiusSm,
            border: `1px solid ${hasApi ? 'rgba(52,211,153,0.25)' : T.border}`,
            background: hasApi ? 'rgba(52,211,153,0.07)' : 'transparent',
            cursor: 'pointer', transition: 'all 0.14s',
          }}>
          <Settings size={11} />{hasApi ? 'Edit API' : 'Set API'}
        </button>
        <button
          onClick={e => { e.stopPropagation(); onDelete(sensor.id) }}
          style={{
            padding: '0.4rem 0.6rem', borderRadius: T.radiusSm,
            border: '1px solid rgba(239,68,68,0.20)',
            background: 'rgba(239,68,68,0.05)',
            color: '#f87171', cursor: 'pointer', transition: 'all 0.14s',
          }}>
          <Trash2 size={11} />
        </button>
      </div>
...
const SidebarSensorRow = ({ s, isSelected, onSelect, onConfigureApi }) => {
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: '0.72rem', fontWeight: 600, color: T.textPrimary, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
          {s.name}
        </div>
        <div style={{ fontSize: '0.58rem', color: T.textSecondary }}>{s.room_name || '—'}</div>
      </div>
      {hasApi && (
        <span style={{ fontSize: '0.50rem', fontWeight: 700, fontFamily: T.mono, color: '#34d399', padding: '1px 5px', borderRadius: T.radiusFull, flexShrink: 0, background: 'rgba(52,211,153,0.10)', border: '1px solid rgba(52,211,153,0.22)' }}>
          LIVE
        </span>
      )}
      ...
      <button
        onClick={e => { e.stopPropagation(); onConfigureApi(s) }}
        style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '2px', color: hasApi ? '#34d399' : T.textMuted, lineHeight: 1, flexShrink: 0 }}>
        <Settings size={11} />
      </button>
    </motion.div>
  )
...
const DEFAULT_API_CONFIG = {
  endpoint: '', mode: 'direct', array_key: 'logs', sort_by: 'created_at', value_path: '', unit: '', refresh_ms: 15000,
}
    ...
    >
      ...
      >
        <button onClick={onClose} style={{
          ...
          padding: '4px 6px', cursor: 'pointer', color: T.textSecondary, lineHeight: 1,
        }}>
          <X size={13} />
        </button>
        {children}
...
function ApiConfigModal({ sensor, onSave, onClose }) {
  ...
  const inputStyle = {
    ...
    fontSize: '0.78rem', fontFamily: T.sans,
    ...
    <ModalOverlay onClose={onClose}>
      ...
      <div style={{ marginBottom: '1.25rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.35rem', flexWrap: 'wrap' }}>
          {QUICK_PRESETS.map(p => (
            <Btn key={p.label} sm ghost onClick={() => setCfg(c => ({ ...c, ...p.config }))} style={{ fontSize: '0.62rem' }}>
              {p.label}
            </Btn>
          ))}
        </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.875rem' }}>
          ...
          <div style={{ display: 'flex', gap: '0.4rem' }}>
            {[{ id: 'direct', label: '📦 Direct', sub: 'JSON object' }, { id: 'array', label: '📋 Array', sub: 'Sort → newest item' }].map(m => (
              <button key={m.id} onClick={() => upd('mode', m.id)} style={{
                flex: 1, padding: '0.55rem 0.5rem', borderRadius: T.radiusSm, cursor: 'pointer',
                border: `1px solid ${cfg.mode === m.id ? T.accent : T.border}`,
                background: cfg.mode === m.id ? `${T.accent}16` : T.surface,
        ...
        {preview !== null && (
          <div style={{
            padding: '0.65rem 0.875rem', borderRadius: T.radiusSm,
            background: testOk ? 'rgba(52,211,153,0.07)' : 'rgba(239,68,68,0.07)',
            border: `1px solid ${testOk ? 'rgba(52,211,153,0.25)' : 'rgba(239,68,68,0.25)'}`,
            ...
            <div>
              <div style={{ fontSize: '0.60rem', fontWeight: 700, marginBottom: 3, color: testOk ? '#34d399' : '#f87171' }}>
                {testOk ? 'Success — value extracted:' : 'Error'}
              </div>
        ...
        <div style={{ display: 'flex', gap: '0.5rem', paddingTop: '0.25rem' }}>
          ...
          <button
            ...
            style={{
              ...
              padding: '0.55rem', borderRadius: T.radiusSm, fontWeight: 700, fontSize: '0.80rem',
  ...
  return (
    <ModalOverlay onClose={onClose}>
      <div style={{ marginBottom: '1.5rem' }}>
        ...
        {pendingPos && (
          <div style={{ fontSize: '0.63rem', fontFamily: T.mono, color: T.textSecondary, marginTop: 4 }}>
      ...
      <form onSubmit={handleSubmit(data => onSave({ ...data, ...pendingPos }))}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.4rem' }}>
              {SENSOR_TYPES.map(t => (
                <button key={t.id} type="button"
                  ...
                  style={{
                    padding: '0.6rem 0.3rem', borderRadius: T.radiusSm, cursor: 'pointer',
                    border: `1px solid ${typeValue === t.id ? t.color + '80' : T.border}`,
                    background: typeValue === t.id ? `${t.color}12` : T.surface,
                    fontSize: '0.60rem', color: typeValue === t.id ? t.color : T.textSecondary,
        ...
        <div style={{ display: 'flex', gap: '0.6rem' }}>
          ...
          <button
            ...
            style={{
              flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.3rem',
              padding: '0.6rem', borderRadius: T.radiusSm, fontWeight: 700, fontSize: '0.80rem',
              background: isValid ? `linear-gradient(135deg, ${T.accent}, #8b5cf6)` : T.surface,
              border: 'none', color: isValid ? '#fff' : T.textMuted, cursor: isValid ? 'pointer' : 'not-allowed',
            }}>
  ...
  return (
    <div style={{
        ...
        {(addMode || editMode) && (
          <motion.div
            ...
            style={{
              ...
              background: addMode ? 'rgba(59,130,246,0.88)' : 'rgba(245,158,11,0.88)',
              color: '#fff', textAlign: 'center', padding: '0.5rem',
              fontSize: '0.72rem', fontWeight: 700, letterSpacing: '0.2px',
            }}>
            {addMode ? '📍 Click on the map to place sensor' : '✋ Drag sensors to reposition'}
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div style={{
        ...
        background: 'var(--bg-surface)', borderRadius: '12px', padding: '0.3rem',
      ...
      <svg ref={svgRef} viewBox="0 0 100 100"
        style={{ width: '100%', aspectRatio: '1.06/1', display: 'block', cursor: addMode ? 'crosshair' : 'default', userSelect: 'none' }}
        ...
        <g transform={`scale(${zoom})`} style={{ transformOrigin: '50px 50px' }}>
          ...
          {ROOMS.map(room => (
            <g key={room.id}>
              ...
              {room.area ? (
                <>
                  <text x={room.x + room.w / 2} y={room.y + room.h / 2 - 2.2} textAnchor="middle" fontSize={2.3}
                    fill="var(--text-primary)" fontFamily="var(--font-body)" fontWeight="700" letterSpacing="0.2">
                    {room.label}
                  </text>
                  <text x={room.x + room.w / 2} y={room.y + room.h / 2 + 2.5} textAnchor="middle" fontSize={1.6}
                    fill="var(--text-secondary)" fontFamily="var(--font-body)">
                    {room.area}
                  </text>
                </>
              ) : (
                <text x={room.x + room.w / 2} y={room.y + room.h / 2} textAnchor="middle" dominantBaseline="central"
                  fontSize={1.8} fill="var(--text-muted)" fontFamily="var(--font-body)"
            ...
            return (
              <g key={sensor.id} transform={`translate(${sensor.x}, ${sensor.y})`} style={{ cursor: editMode ? 'grab' : 'pointer' }}>
                ...
                <circle r={2.2}
                  ...
                  stroke={meta.color} strokeWidth={0.65}
                  style={{ filter: isSelected ? `drop-shadow(0 0 4px ${meta.color})` : 'drop-shadow(0 2px 4px rgba(15,23,42,0.12))' }}
                ...
                {!editMode && !isSelected && (
                  <g transform="translate(2.8, -4.2)">
                    <rect x={0} y={0} width={sensor.name.length * 1.05 + 2.8} height={3.9}
                      rx={2.5} fill="var(--bg-surface)" stroke="var(--border)" strokeWidth={0.38} />
                    <text x={1.4} y={2.6} fontSize={1.65} fill="var(--text-primary)"
                      fontFamily="var(--font-body)" fontWeight="600" letterSpacing="0.08">
                      {sensor.name}
                    </text>
                  </g>
                )}
                <text textAnchor="middle" dominantBaseline="central" fontSize={1.5} style={{ userSelect: 'none' }}>
                  {meta.icon}
                </text>
...
export default function FloorplanPage() {
    ...
    try {
      if (!isOn) {
        ...
        if (res.status === 403) {
          ...
          showToast(err.reason || 'Boiler is blocked during this period.', 'error')
          return
        }
        if (!res.ok) throw new Error('Server error')
        setIsOn(true)
        showToast('Boiler ON', 'success')
      } else {
        await api.manualBoiler(false)
        setIsOn(false)
        showToast('Boiler OFF', 'success')
      }
    } catch {
      showToast('Error changing boiler state', 'error')
    }
  ...
  const toggleThermostat = useCallback(async () => {
    try {
      if (thermostat) {
        ...
        setThermostat(false)
        showToast('Thermostat disabled', 'success')
      } else {
        ...
        if (!res.ok) throw new Error('Server error')
        setThermostat(true)
        showToast('Thermostat enabled', 'success')
      }
    } catch {
      showToast('Error changing thermostat state', 'error')
    }
  ...
  const adjustTarget = useCallback(async (delta) => {
    ...
    setTargetTemp(next)
    try {
      await api.setTargetTemp(next)
      showToast(`Target set to ${next}°C`, 'success')
    } catch {
    ...
    <div style={{ minHeight: '100vh', background: T.bg, color: T.textPrimary, fontFamily: T.sans, padding: '0 1.5rem 2rem' }}>
      ...
      <motion.div
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <div style={{
            ...
            background: `linear-gradient(135deg, ${T.accent}18, rgba(245,158,11,0.18))`,
          ...
          <div>
            <h1 style={{ margin: 0, fontSize: '1.15rem', fontWeight: 800, color: T.textPrimary, letterSpacing: '-0.3px' }}>
              Smart Floorplan
            </h1>
            <p style={{ margin: 0, fontSize: '0.65rem', color: T.textMuted, letterSpacing: '0.2px' }}>
              Live sensor monitoring · auto-refresh 10s
            </p>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', flexWrap: 'wrap' }}>
          ...
          <Btn sm active={editMode} ghost={!editMode} onClick={() => { setEditMode(e => !e); setAddMode(false); setSelectedSensor(null) }}>
            <Move size={12} /> {editMode ? 'Done' : 'Move'}
          </Btn>
          <Btn sm active={addMode} ghost={!addMode} onClick={() => { setAddMode(a => !a); setEditMode(false); setSelectedSensor(null) }}>
            <Plus size={12} /> {addMode ? 'Cancel' : 'Add'}
          </Btn>

          <input ref={fileRef} type="file" accept="image/*" style={{ display: 'none' }} onChange={e => uploadBg(e.target.files?.[0])} />
          <Btn sm ghost onClick={() => fileRef.current?.click()}>
            <Home size={12} /> Map
          </Btn>
          {bgImage && (
            <Btn sm ghost danger onClick={() => { setBgImage(''); localStorage.removeItem('fp-bg-image') }}>
              <X size={11} />
            </Btn>
          )}
            ...
            {alerts.map(s => (
              <motion.div key={s.id}
                ...
                style={{
                  ...
                  padding: '0.4rem 0.875rem', borderRadius: T.radiusFull,
                  background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.22)',
                  fontSize: '0.68rem', color: '#f87171', fontFamily: T.mono, fontWeight: 600,
                }}>
      ...
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.12 }} style={{ marginBottom: '1rem' }}>
        <BoilerControlCard
          ...
          onToggleThermostat={toggleThermostat}
      ...
      {viewMode === 'map' ? (
        <motion.div
          ...
          style={{ display: 'grid', gridTemplateColumns: '1fr 260px', gap: '1rem', marginBottom: '1.5rem' }}>
          ...
          {/* Sidebar */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', maxHeight: 'calc(100vh - 280px)', position: 'sticky', top: 72 }}>

            {/* Legend */}
            <GCard style={{ padding: '0.875rem' }}>
            ...
            <GCard style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0, overflow: 'hidden' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.6rem 0.875rem', borderBottom: `1px solid ${T.border}` }}>
                <Wifi size={12} style={{ color: T.accent }} />
                <span style={{ fontSize: '0.70rem', fontWeight: 700, color: T.textPrimary }}>Sensors</span>
                <span style={{ marginLeft: 'auto', fontSize: '0.58rem', fontFamily: T.mono, color: T.textMuted, padding: '1px 6px', borderRadius: T.radiusFull, background: T.surface, border: `1px solid ${T.border}` }}>
                  {sensors.length}
                </span>
              </div>
                ...
                <AnimatePresence mode="popLayout">
                  {loading ? (
                    ...
                    <SidebarSensorRow
                      ...
                      onSelect={s => { setSelectedSensor(s); setEditMode(false); setAddMode(false) }}
        ...
        <motion.div
          initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.25 }}
          style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: '0.875rem', marginBottom: '1.5rem' }}>
          <AnimatePresence mode="popLayout">
            {loading ? (
              ...
              <div style={{ gridColumn: '1/-1', textAlign: 'center', padding: '3rem', color: T.textSecondary, fontSize: '0.80rem' }}>
                ...
                <div style={{ marginTop: '0.75rem' }}>
                  <Btn onClick={() => setAddMode(true)}>+ Add first sensor</Btn>
                </div>
              </div>
            ) : sensors.map(s => (
      ...
      {/* ── MODALS ── */}
      {modal && (
        <SensorFormModal sensor={modal.sensor} pendingPos={modal.pendingPos} onSave={saveSensor} onClose={() => setModal(null)} />
      )}
      {apiModal && (
        <ApiConfigModal sensor={apiModal} onSave={saveApiConfig} onClose={() => setApiModal(null)} />
      )}
      {blackoutOpen && (
        <BoilerBlackoutModal
      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        @media (max-width: 768px) {
          .fp-widgets-grid { grid-template-columns: 1fr !important; }
        }
      `}</style>
    </div>
  )
